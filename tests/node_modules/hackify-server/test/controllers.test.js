...
angular.module('socket.services.mock', [])
.factory('socketService', function($rootScope) {
  ...
  emit: function(id, eventName, data){
    if(!this.emits[eventName])
      this.emits[eventName] = [];
    this.emits[eventName].push(data);
  },
  expect: function(eventName, emitData, receiveData) {
    if(!this.expects[eventName])
      this.expects[eventName] = [];
    this.expects[eventName].push({emitData: emitData, receiveData: receiveData});
  },
  //simulate an inbound message to the socket from the server (only called from the test)
  receive: function(eventName){
    var sockMock = this;
    angular.forEach(this.events[eventName], function(callback){
        // Get data supposed to receive
        angular.forEach(sockMock.emits[eventName], function(emitData){
          angular.forEach(sockMock.expects[eventName], function(expect){
            if (JSON.stringify(expect.emitData) === JSON.stringify(emitData))
              $rootScope.$apply(function() {
                callback.apply(sockMock, [expect.receiveData]);
              });
          });
        });
      });
